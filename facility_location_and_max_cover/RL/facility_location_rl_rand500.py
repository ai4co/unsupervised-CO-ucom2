...
data_train = torch.load(f"facility_location_rand500_train.pt", map_location=device)
locs_a = data_train.unsqueeze(-2)
locs_b = data_train.unsqueeze(-3)
distances_train = ((locs_a - locs_b) ** 2.0).sum(-1)

data_test = torch.load(f"facility_location_rand500_train.pt", map_location=device)
