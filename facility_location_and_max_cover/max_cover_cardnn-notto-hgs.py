...
model_path = f"saved_models/max_covering_cardnn-hgs_{cfg.train_data_type}_{cfg.train_max_covering_items}-{cfg.num_sets}-{cfg.num_items}_{timestamp_}.pt"
...
for index, (name, weights, sets) in enumerate(dataset):
    ...
    best_obj, best_top_k_indices = sinkhorn_max_covering_fast_noTTO(
        weights,
        sets,
...
wb.save(
    f"res/max_covering-cardnn-notto-hgs-{cfg.test_data_type}_{cfg.test_max_covering_items}-{cfg.num_sets}-{cfg.num_items}_{get_local_time()}.xls"
)
