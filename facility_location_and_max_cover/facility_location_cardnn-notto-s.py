...
model_path = f"saved_models/facility_location_cardnn-s_{cfg.train_data_type}_{cfg.train_num_centers}-{cfg.num_data}_saved_{timestamp_}.pt"
...
for index, (prob_name, points) in enumerate(dataset):
    ...
    print(
        f"{prob_name} CardNN-s objective={objective:.4f} selected={sorted(selected_indices.cpu().numpy().tolist())} time={finish_time}"
    )
    if index == 0:
...
wb.save(
    f"res/facility_location-cardnn-notto-s-{cfg.test_data_type}-{cfg.test_num_centers}-{timestamp_true}.xls"
)
