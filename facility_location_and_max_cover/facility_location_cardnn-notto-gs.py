...
model_path = f"saved_models/facility_location_cardnn-gs_{cfg.train_data_type}_{cfg.train_num_centers}-{cfg.num_data}_saved_{timestamp_}.pt"
...
if not os.path.exists(model_path):
    ...
    for epoch in range(cfg.train_iter):
        ...
        for index, (_, points) in enumerate(train_dataset):
            graph, dist = build_graph_from_points(
                points, None, True, cfg.distance_metric
            )
            ...
            obj = compute_objective_differentiable(
                dist, probs, temp=50
            )  # set smaller temp during training
            obj.mean().backward()
            obj_sum += obj.mean()
...
for index, (prob_name, points) in enumerate(dataset):
    ...
    objective, selected_indices, finish_time = sinkhorn_facility_location_noTTO(
        points,
        cfg.test_num_centers,
        model,
        cfg.softmax_temp,
        cfg.gumbel_sample_num,
        cfg.gumbel_sigma,
        cfg.sinkhorn_tau,
        cfg.sinkhorn_iter,
        cfg.gs_opt_iter,
    ...
    print(
        f"{prob_name} CardNN-GS objective={objective:.4f} selected={sorted(selected_indices.cpu().numpy().tolist())} time={finish_time}"
    )
    if index == 0:
...
wb.save(
    f"res/facility_location-cardnn-notto-gs-{cfg.test_data_type}-{cfg.test_num_centers}-{timestamp_true}.xls"
)
